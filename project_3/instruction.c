...
int Instruction_Map(int pid, int va, int value_in)
{
    ...
    if (pa == -1)
    {
        printf("Error: Unable to find free PFN.\n");
        // If no empty page was found, we must evict a page to make room
        pa = PT_Evict();
        if (pa == -1)
        {
            printf("Error: Unable to evict page.\n");
        }
    ...
    {
        pa = PT_PageTableInit(pid, pa);
        if (pa == -1)
        {
            printf("Error: Page table not initialized.\n");
            return -1;
        }
    }
...
int Instruction_Store(int pid, int va, int value_in)
{
    ...
    pa = MMU_TranslateAddress(pid, VPN(va), 16);

    if (pa == -1)
    {
        printf("Error: Error translating virtual address into physical address.\n");
        return -1;
    }
